...
def get_template_config(template_id):
    ...
    config = {"vlans": [], "firewall_rules": {}, "ssids": []}
    ...
    if template_config["firewall_rules"]:
        ...
        try:
            dashboard.appliance.updateNetworkApplianceFirewallL3FirewallRules(
                network_id, rules=template_config["firewall_rules"]["rules"]
            )
    ...
    if template_config["ssids"]:
        ...
        for ssid in template_config["ssids"]:
            try:
                ...
                ssid_config = {k: v for k, v in ssid.items() if k not in ["number", "adminSplashUrl"]}

                dashboard.wireless.updateNetworkWirelessSsid(network_id, number=ssid["number"], **ssid_config)
